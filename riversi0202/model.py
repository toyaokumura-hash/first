...
# なので再計算させた方が楽だし最新の情報を使える
    ...
    def flip_judge(self,top :int) -> bool:
        """
        次のターンに
        盤面にコマを置けるマスがあるか判定し、なければパスにしたい
        """
        for board_num,board_e in enumerate(self.board): #ここでボードの値をマス番号、マス情報に分ける
                if board_e == self.EMPTY: #マス情報が空白のものだけ取り出し
                        tops = self.flip_list(board_num,top) #flip_listにtopsを計算させている。
                        # 今回はマスを置いたコマではなく空白マスで、置いたコマの色ではなく相手の色で計算している。Controllerのput参照
                        if tops :
                                return True #topsに一つでもtrueがあればいいのでループの中で計算している

        return False #置けるマスがない、空白がない


#     def flip_judge2(self,top :int) -> bool:
#         """
#         上で盤面にコマを置けるマスがなかった場合、勝敗判定に入るため
#         どの色でもコマを置けないのか判定したい
#         """
#         for board_num,board_e in enumerate(self.board): #ここでボードの値をマス番号、マス情報に分ける
#                 if board_e == self.EMPTY: #マス情報が空白のものだけ取り出し
...
#                 return False
    ...
    def win_judge(self) :
        """
        どの色でもコマを置けなかった場合、勝敗判定に入りたい
        """
        from collections import Counter
        board_judge = Counter(self.board)
        b = board_judge.get(self.BLACK1, 0) #どちらかが0個になった時に判定されなかったので、0個を判定できるように
        w = board_judge.get(self.WHITE2, 0) #getで変数を作る
        # print("win")
        match (b, w):
                case (b, w) if w > b:
                        print(f"白の勝ち！（白：{w}個、黒：{b}個）")
                case (b, w) if w < b:
                        print(f"黒の勝ち！（白：{w}個、黒：{b}個）")
                case (b, w) if b == w:
                        print("引き分け!")
