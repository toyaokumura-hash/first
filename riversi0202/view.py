...
# クラスの定義ここでは画面の要素のサイズ感がメイン
    ...
    BLACK1 = ReversiController.BLACK1
    WHITE2 = ReversiController.WHITE2
    CELL_WIDTH = 50
    # マスの横幅
    CELL_HEIGHT = 50
    # マスの高さ
    ...
    MARGIN = 5
    # リバーシのウインドウと実線の隙間
    BD_WIDTH = CELL_WIDTH * 3 + MARGIN * 2
    # 盤面全体の横幅、８マス＋両橋ってこと
    BD_HEIGHT = CELL_HEIGHT * 3 + MARGIN * 2
    # 盤面の縦幅
...
# 同じ条件で縦幅をyとして
    ...
    def draw_top(self,x: int,y: int,top: int) -> None:
        ...
        top_color = "white" if top == self.WHITE2 else "black"
...
# 左端が X なら、右端は X+1って考えはグリッドを扱う上で結構大事らしい
    ...
    def flash(self) -> None:
        ...
        for y in range(self.MARGIN,self.MARGIN + self.CELL_WIDTH * 3  + 1, self.CELL_WIDTH):
            self.canvas.create_line(self.MARGIN, y, self.MARGIN + self.CELL_WIDTH * 3, y)
        for x in range(self.MARGIN, self.MARGIN + self.CELL_HEIGHT * 3  + 1, self.CELL_HEIGHT):
            self.canvas.create_line(x, self.MARGIN, x, self.MARGIN + self.CELL_HEIGHT * 3)
        ...
        for x in range(3):
            for y in range(3):
                match self.controller.board[x][y]:
                    case ReversiController.BLACK1:
                        self.draw_top(x, y, ReversiController.BLACK1)
                    case ReversiController.WHITE2:
                        self.draw_top(x, y, ReversiController.WHITE2)
