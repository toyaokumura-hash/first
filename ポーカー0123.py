...
def judge_hand(hand):
    ...
    for hand_num in hand:
        cnt[hand_num] = cnt.get(hand_num,0) + 1
            # ↑手札を数えるやつ
...
def change_hand(hand: list[int]) -> list[int]: # 入力された手札を捨てて、新しく手札を追加したい
        ...
        deck: list[int] = [_ for _ in range(1,14)] * 4
        new_hand: list[int] = random.sample(deck,change_cnt)
...
try: 	change_num = input("入れ替えたい手札を1〜5で半角スペースを入れて選んでね\n入れ替えない時はEnterを押してね >> ")
except ValueError:  print("数字かEnterで入力してや")
